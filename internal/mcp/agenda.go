...
func AgendaTools() []Tool {
	...
	return []Tool{
		...
		{Name: "agenda.create_event", Description: "Cria diretamente um evento quando esta ferramenta é chamada com argumentos válidos. O host não exige frase específica de pedido nem confirmação adicional para criação. O usuário pode falar datas por extenso; converta os argumentos antes da chamada. Esclareça horário, data, ano, duração ou título ausente/ambíguo. Envie start e end em RFC3339 com offset explícito, por exemplo {\"title\":\"pocket\",\"start\":\"2030-05-20T09:00:00-03:00\",\"end\":\"2030-05-20T10:00:00-03:00\"}. Esse exemplo ilustra uma hora, não valores padrão. Calcule end pela duração informada, posterior a start. Não peça formatos técnicos ao usuário. Nunca anuncie sucesso antes de status created.", InputSchema: json.RawMessage(`{"type":"object","properties":{` + interval + `,"title":{"type":"string","minLength":1,"maxLength":256}},"required":["start","end","title"],"additionalProperties":false}`)},
