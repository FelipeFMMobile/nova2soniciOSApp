...
func AgendaTools() []Tool {
	...
	return []Tool{
		...
		{Name: "agenda.create_event", Description: "Cria evento após pedido explícito do usuário para agendar, sem confirmação adicional. O usuário pode falar datas por extenso; você deve converter os argumentos antes da chamada. Esclareça horário, data, ano, duração ou título ausente/ambíguo. Envie start e end no formato RFC3339 AAAA-MM-DDTHH:MM:SS±HH:MM, com offset explícito; nunca envie datas por extenso ou apenas DD/MM/AAAA nos argumentos. Exemplo: para pocket em 20 de maio de 2030 às 09:00, com duração de uma hora em America/Sao_Paulo, envie {\"title\":\"pocket\",\"start\":\"2030-05-20T09:00:00-03:00\",\"end\":\"2030-05-20T10:00:00-03:00\"}. O exemplo não define valores padrão. Calcule end a partir da duração informada; end deve ser posterior a start. Não peça ao usuário que dite um formato numérico técnico. Nunca anuncie sucesso antes de status created.", InputSchema: json.RawMessage(`{"type":"object","properties":{` + interval + `,"title":{"type":"string","minLength":1,"maxLength":256}},"required":["start","end","title"],"additionalProperties":false}`)},
